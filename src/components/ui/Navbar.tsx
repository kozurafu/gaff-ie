      ...
      <nav className={`sticky top-0 z-50 transition-all duration-300 ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-[60px]">
            ...
            {/* Center nav — Daft-style category tabs */}
        ...
        <div className="fixed inset-0 z-40 md:hidden" onClick={() => setMenuOpen(false)}>
          ...
          >
            <div className="p-5 pt-20 space-y-1">
              {NAV_LINKS.map((link) => (
                <a key={link.label} href={link.href} className="block px-4 py-3 rounded-lg text-sm font-semibold text-gaff-slate-600 hover:bg-gray-50">
                  {link.label}
                </a>
              ))}
              ...
              {!loading && user ? (
                <>
                  ...
                  <button onClick={handleLogout} className="block w-full text-left px-4 py-3 rounded-lg text-sm font-medium text-red-500 hover:bg-red-50">
                    Sign Out
                  </button>
                </>
              ) : !loading ? (
                <>
                  <a href="/auth/login" className="block px-4 py-3 rounded-lg text-sm font-medium text-gaff-slate-600 hover:bg-gray-50">Sign In</a>
                  <a href="/auth/register" className="block mx-4 mt-2 text-center py-3 text-sm font-semibold bg-gaff-teal text-white rounded-lg">Sign Up</a>
                </>
              ) : null}
            </div>
          </div>
